...
def test_protocol_payload_exposes_tokens_and_frozen_metadata() -> None:
    ...
    assert first["target_occurrences"] == 2
...
def test_save_recording_writes_wav_and_metadata(tmp_path: Path) -> None:
    ...
    assert metadata["target_occurrences"] == 2
    assert metadata["spoken_pattern"] == "zha a zha a"


def test_load_session_restores_saved_protocol_and_completed_recordings(tmp_path: Path) -> None:
    protocol = live_calibration_protocol()
    ...
    assert loaded["protocol"]["prompts"][0]["spoken_pattern"] == "zha a zha a"
    assert loaded["recordings"][0]["wav"] == "001_zh_plain.wav"
    assert recording_path_for_session(session_dir, 0).name == "001_zh_plain.wav"


def test_load_session_rejects_non_session_folder(tmp_path: Path) -> None:
    try:
        load_calibration_session(tmp_path)
    except ValueError as exc:
        assert "calibration session" in str(exc)
    else:
        raise AssertionError("expected non-session folder to be rejected")
