...
@dataclass(frozen=True)
class NasalWindowResult:
    name: str
    start_fraction: float
    end_fraction: float
    pairwise: tuple[NasalPairResult, ...]


@dataclass(frozen=True)
class NasalRoleResult:
    role: str
    counts: dict[str, int]
    windows: tuple[NasalWindowResult, ...]
...
def _features(segment: AudioSegment) -> NasalFeatures:
    ...
    mask = (freqs >= 100.0) & (freqs <= upper)
    band_freqs = freqs[mask]
    band_power = power[mask]
    total = float(np.sum(band_power))
    if total <= 1e-16:
        raise AudioReadError("nasal window has no usable spectral energy")

    weights = band_power / total
    low_mask = band_freqs < 600.0
    mid_mask = (band_freqs >= 600.0) & (band_freqs < 2000.0)
    high_mask = band_freqs >= 2000.0
    low_energy = float(np.sum(band_power[low_mask])) + 1e-18
    mid_energy = float(np.sum(band_power[mid_mask])) + 1e-18

    return NasalFeatures(
        low_ratio=low_energy / total,
        mid_ratio=mid_energy / total,
        high_ratio=float(np.sum(band_power[high_mask]) / total),
        centroid_hz=float(np.sum(band_freqs * weights)),
        spectral_flatness=float(
            np.exp(np.mean(np.log(band_power))) / np.mean(band_power)
        ),
        periodicity=_periodicity(segment.samples, segment.sample_rate),
        low_peak_hz=_peak_frequency(freqs, power, 150.0, 600.0),
        mid_peak_hz=_peak_frequency(freqs, power, 600.0, 2000.0),
        low_mid_balance_db=float(10.0 * np.log10(low_energy / mid_energy)),
    )


def _matrix(samples: list[NasalSample], window: str) -> np.ndarray:
    return np.vstack([getattr(sample, window).vector() for sample in samples])


def _cross_oto_set(
    grouped: dict[str, list[NasalSample]],
    left: str,
    right: str,
    window: str,
...
def _window_result(
    ...
    adjusted = _holm_adjust(
        [item.stratified_permutation_p for item in results]
    )
    results = [
        replace(item, stratified_p_holm=adjusted[index])
        for index, item in enumerate(results)
    ]
    return NasalWindowResult(
        name=name,
...
def analyze_nasal(root: Path, base_unit: str = "m") -> NasalAnalysis:
    if base_unit not in {"m", "n"}:
        raise ValueError("nasal analyzer supports only m and n")
    ...
    resolved, duplicate_observations_removed, ambiguous_segments_removed = (
        _resolve_candidates(candidates)
    )

    skipped = 0
    grouped: dict[str, dict[str, list[NasalSample]]] = defaultdict(
        lambda: defaultdict(list)
    )
    for candidate in resolved:
        try:
            whole = consonant_segment(candidate.entry, edge_trim=0.04)
            core = _features(slice_segment(whole, 0.18, 0.48))
            late = _features(slice_segment(whole, 0.60, 0.90))
    ...
    return NasalAnalysis(
        ...
        ambiguous_segments_removed=ambiguous_segments_removed,
        roles=tuple(roles),
    )
